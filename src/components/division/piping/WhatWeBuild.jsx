  ...
  return (
    <section className="border-b border-line bg-ink">
      <div className="max-w-[1440px] mx-auto px-[clamp(20px,4vw,40px)] py-[clamp(56px,8vw,120px)] flex flex-col gap-12">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(min(300px,100%),1fr))] gap-6 items-start">
          {data.families.map((fam) => (
            <div
              key={fam.title}
              className={`border bg-panel p-8 flex flex-col gap-5 transition-colors duration-150 ${
          ...
          <div className="border border-line bg-panel p-8 flex flex-col gap-5">
            ...
            <div className="flex flex-col border-t border-line">
              {data.materials.rows.map(([name, spec]) => (
                <div
                  key={name}
