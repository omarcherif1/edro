...
function StandardCard({ code, label }) {
  return (
    <div className="border border-line bg-panel px-5 py-[18px] flex flex-col gap-1.5 transition-colors duration-150 hover:border-accent">
      <span className="font-mono text-[17px] tabular-nums text-fg">
        {code}
      </span>
      <span className="text-[12px] leading-[1.5] text-muted">{label}</span>
    </div>
  );
}
  ...
  return (
    <section className={`border-b border-line ${BG[bg]}`}>
      <div className="max-w-[1440px] mx-auto px-[clamp(20px,4vw,40px)] py-[clamp(56px,8vw,120px)] flex flex-col gap-10">
        ...
        <div className="hidden md:grid grid-cols-[repeat(auto-fit,minmax(210px,1fr))] gap-4">
          {standards.map(([code, label]) => (
            <StandardCard key={code} code={code} label={label} />
          ))}
        </div>

        <MobileCarousel
          items={standards.map(([code, label]) => (
            <StandardCard key={code} code={code} label={label} />
          ))}
        />
      </div>
